...
- `src/SmartphoneContainer.js` is the JavaScript entry point.
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/index.css`. It would still be imported from `src/SmartphoneContainer.js`, but you could always remove that import if you later migrate to a different build tool.
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the beginning of your `src/SmartphoneContainer.js` file:
...
Finally, import the newly created `.scss` file instead of the default Bootstrap `.css` in the beginning of your `src/SmartphoneContainer.js` file, for example:
...
following in their [`src/SmartphoneContainer.js`](src/SmartphoneContainer.js) file:
...
- If the watcher doesn’t see a file called `SmartphoneContainer.js` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebook/create-react-app/issues/1164) due to a Webpack bug.
